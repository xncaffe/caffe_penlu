...
void PENLULayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  CHECK_GE(bottom[0]->num_axes(), 2)
      << "Number of axes of bottom blob must be >=2.";
  PENLUParameter penlu_param = this->layer_param().penlu_param();
  int channels = bottom[0]->channels();
  channel_shared_ = penlu_param.channel_shared();
  ...
  if (this->blobs_.size() > 0) {
    ...
    if (channel_shared_) {
      ...
      this->blobs_[2].reset(new Blob<Dtype>(vector<int>(0)));//
    } else {
      this->blobs_[0].reset(new Blob<Dtype>(vector<int>(1, channels)));
      this->blobs_[1].reset(new Blob<Dtype>(vector<int>(1, channels)));
      this->blobs_[2].reset(new Blob<Dtype>(vector<int>(1, channels)));//
    }
    shared_ptr<Filler<Dtype> > filler;
    if (penlu_param.has_alpha_filler()) {
      filler.reset(GetFiller<Dtype>(penlu_param.alpha_filler()));
    } else {
    ...
    if (penlu_param.has_beta_filler()) {
      filler.reset(GetFiller<Dtype>(penlu_param.beta_filler()));
    } else {
      FillerParameter filler_param;
      filler_param.set_type("constant");
    ...
    if (penlu_param.has_eta_filler()) {
      filler.reset(GetFiller<Dtype>(penlu_param.eta_filler()));
    } else {
      FillerParameter filler_param;
      filler_param.set_type("constant");
      filler_param.set_value(1);
  ...
  if (channel_shared_) {
    ...
    CHECK_EQ(this->blobs_[2]->count(), 1)
        << "Eta  size is inconsistent with prototxt config";
  } else {
  ...
  backward_buff_eta.Reshape(vector<int>(1, bottom[0]->count(1)));
  caffe_set(multiplier_.count(), Dtype(1), multiplier_.mutable_cpu_data());
}

template <typename Dtype>
void PENLULayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  CHECK_GE(bottom[0]->num_axes(), 2)
      << "Number of axes of bottom blob must be >=2.";
...
template <typename Dtype>
void PENLULayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
    const vector<Blob<Dtype>*>& top) {
  ...
  const Dtype* eta  = this->blobs_[2]->cpu_data();
  ...
  for (int i = 0; i < count; ++i) {
    ...
    top_data[i] = ( pow( (std::max(bottom_data[i], Dtype(0))),eta[c]) * (bottom_data[i] > 0)
        + alpha[c] * ( exp(beta[c]*std::min(bottom_data[i], Dtype(0))) - 1) * (bottom_data[i] <= 0) );
  }
}

template <typename Dtype>
void PENLULayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
  ...
  if (this->param_propagate_down_[0]) {
    ...
    Dtype* eta_diff  = this->blobs_[2]->mutable_cpu_diff();
    ...
    for (int i = 0; i < count; ++i) {
      ...
      eta_diff[c]  += top_diff[i] * ( top_data[i]*(log(bottom_data[i])/log(exp(1)) ) * (bottom_data[i] > 0) );
      bottom_diff[i] = top_diff[i] * ( ((eta[c]*top_data[i])/bottom_data[i]) * (bottom_data[i] > 0) + ( beta[c]*( top_data[i] + alpha[c] ) ) * (bottom_data[i] <= 0) );
    }
  }
...
INSTANTIATE_CLASS(PENLULayer);
REGISTER_LAYER_CLASS(PENLU);
